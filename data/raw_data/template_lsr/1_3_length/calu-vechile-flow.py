...
with open('anon_1_3_700_0.3_synthetic.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
with open('anon_1_3_700_0.3_synthetic.json', 'r', encoding='utf-8') as f2:
    data2 = json.load(f2)
...
print(all_road_vehicle)

all_r_end = []
r_end = {}
for t in data:
    if t['route'][-1] not in all_r_end:
        all_r_end.append(t['route'][-1])
        r_end.setdefault(t['route'][-1], 1)
    else:
        r_end[t['route'][-1]] = r_end[t['route'][-1]]+1
print(r_end)
for idx,t in enumerate(data):
    if 'road_3_1_0' in t['route']:
        del data[idx]
# 将数据保存为JSON文件
